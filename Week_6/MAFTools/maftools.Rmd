...
BiocManager::install("BSgenome.Hsapiens.UCSC.hg19", lib="/home/plott/R/x86_64-conda-linux-gnu/4.4", force=TRUE)
```
...
![](overview.png) Besides the MAF files, maftools also facilitates processing of BAM files. Please refer to below vignettes and sections to learn more.
...
```{r summaryPlot,fig.height=4, fig.width=6}
...
```{r oncoplot, fig.align='left',fig.height=3.5,fig.width=6, fig.align='left'}
...
```{r titv, fig.height=3, fig.width=4.2, eval = T, fig.align='left'}
...
```{r lollipopPlot,fig.align='left', fig.width=4.5, fig.height=2.5}
...
```{r lollipopPlot4, fig.align='left', fig.width=4.5, fig.height=2.5}
#example data
...
```{r plotProtein,fig.align='left', fig.width=5, fig.height=1.2}
...
```{r rainfallPlot, results='hide', message=FALSE}
brca <- system.file("extdata", "brca.maf.gz", package = "maftools")
brca = read.maf(maf = brca, verbose = FALSE)
```

```{r rainfallPlot2, fig.height=3,fig.width=6,fig.align='left'}
...
```{r tcgaCompare, fig.align='left', fig.height=3.25, fig.width=6, message=FALSE, results='hide'}
...
```{r plotVaf, fig.align='left', fig.height=3, fig.width=3}
...
```{r gisticChromPlot, fig.width=4, fig.height=3, fig.align='left'}
...
```{r coGisticChromPlot, fig.width=4, fig.height=5, fig.align='left'}
...
```{r gisticBubblePlot, fig.width=4, fig.height=3, fig.align='left'}
...
```{r gisticOncoPlot, fig.align='left',fig.width=5, fig.height=3, eval=T}
...
```{r somaticInteractions, message=FALSE, fig.height=5, fig.width=5}
...
```{r oncodrive, fig.align='default', fig.width=7,fig.height=5, message=F,results='hide', eval=T}
...
```{r plotOncodrive, fig.align='left', fig.width=3.2, fig.height=3.2}
...
```{r pfamDomains, fig.align='left', fig.width=4, fig.height=3}
...
```{r mafSurvival, fig.width=3, fig.height=3}
...
```{r mafSurvGroup, fig.width=3, fig.height=3}
...
```{r forestPlot, fig.width=6, fig.height=4.5, fig.align='left'}
...
```{r coOncoplot, fig.height=2.5,fig.width=6, eval=T, fig.align='left'}
...
```{r coBarplot, fig.height=3, fig.width=4}
...
```{r lollipopPlot2, warning=FALSE, message=FALSE,fig.align='left', results='hide', fig.height=3.5, fig.width=5}
...
```{r plotEnrichmentResults, fig.width=4, fig.height=3}
...
```{r drugInteractions, fig.height=3, fig.width=5}
...
```{r OncogenicPathways, fig.width=7, fig.height=6}
pws = pathways(maf = laml, plotType = 'treemap')
```

Its also possible to visualize the results

```{r PlotOncogenicPathways, fig.width=6, fig.height=3.5}
...
```{r inferHeterogeneity, echo = TRUE, fig.align='left', fig.height=3.5, fig.width=4, eval=T}
...
```{r plotClusters, fig.align='left', fig.height=3.5, fig.width=4, eval=T}
...
```{r trinucleotideMatrix, eval=TRUE}
...
```{r plotApobecDiff, eval=TRUE, fig.height=3, fig.width=5}
...
```{r estimateSignatures, fig.height=5, fig.width=5, eval=FALSE, message=FALSE}
library('NMF')
laml.sign = estimateSignatures(mat = laml.tnm, nTry = 6)
```

```{r estimateSignatures2, fig.height=3, fig.width=3, eval=TRUE, message=FALSE, echo=FALSE, include=FALSE}
...
```{r plotCophenetic, fig.width=3, fig.height=3, eval=TRUE}
...
```{r extractSignatures, eval=FALSE}
laml.sig = extractSignatures(mat = laml.tnm, n = 3)
```

```{r extractSignatures2, eval=TRUE, echo=FALSE}
...
```{r pheatmap, fig.width=7, fig.height=2.5, fig.align='center', eval=TRUE}
library('pheatmap')
...
```{r plotSignatures, fig.width=6, fig.height=4, fig.align='center', eval = TRUE}
...
stad_mc3 = tcgaLoad(study = "STAD")
stad_mc3

# Change the source to Firehose
stad_fh = tcgaLoad(study = "STAD", source = "Firehose")
stad_fh
```
...
```{r, eval=FALSE}
BiocManager::install(pkgs = "PoisonAlien/TCGAmutations", lib="/home/plott/R/x86_64-conda-linux-gnu/4.4", force=TRUE)
```
